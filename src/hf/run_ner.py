...
from datasets import ClassLabel, DatasetDict, Dataset, load_dataset, load_metric
...
FEW_SHOT_DIR = "data/ner/few_shot"
...
class ModelArguments:
    ...
    use_head_initialization: bool = field(
        default=False,
        metadata={
            "help": "Use QA bi-encoder head to initialize NER prediction head. Will auto-detect head based on |model_name_or_path|."
        },
    )
...
class DataTrainingArguments:
    ...
    downsample: float = field(
            default=None,
            metadata={"help": "Downsample training data by this fraction"}
    )

    few_shot_seed: int = field(
            default=0,
            metadata={"help": "Seed for few-shot training data"}
    )
...
def load_few_shot(dataset_name, few_shot_seed):
    prefix, dataset_name = dataset_name.split("-")
    assert prefix == "5shot"
    if dataset_name in ["conll2003", "wnut_17"]:
        # Re-use dev/test from huggingface datasets
        dataset_dict = load_dataset(dataset_name)
        if dataset_name == "conll2003":
            dataset_dict = dataset_dict.remove_columns(["chunk_tags", "pos_tags"])
        features = dataset_dict["train"].features
    else:
        raise NotImplementedError
    dataset_dict["train"] = load_few_shot_file(dataset_name, few_shot_seed, features=features)
    return dataset_dict
